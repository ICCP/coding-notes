...
\marginnote{Modern terminal emulators all have a feature called \emph{tab completion}; enter part of a command \& hit the \texttt{TAB} key and the terminal will attempt to fill in the rest of the command for you.}
\begin{lstlisting}[style=prompt, nolol]
...
\begin{itemize}
  ...
  \item Dividing integers \textcolor{red}{\textbf{results in an integer}}\index{Fortran!integer division}\footnote{Unexpected integer divison is a tricky problem to find and one of the most common bugs in ICCP.}, so \texttt{3 / 2 == 1} instead of 1.5 as you might expect.
