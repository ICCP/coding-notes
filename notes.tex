...
\lstinputlisting[float=htbp,label=lst:myProg]{examples/myProg.f90}
...
\begin{itemize}
  ...
  \item \texttt{\keyword{integer} :: fibonacci(N)} allocates an array of 20 integers, with the array index\footnote{Fortran array indices can start wherever you want them to, though the default is to start at 1.} running from 1 to 20.
...
\lstinputlisting[float=htbp,label=lst:snppic]{examples/snppic.f}
...
\lstinputlisting[language=,showtabs=true,float=hp,label=lst:example_makefile]{examples/example_makefile}
...
\lstinputlisting[caption=PLplotDemo (continued),float=htbp,label=lst:PLplotDemo,firstline=44,firstnumber=44]{examples/PLplotDemo.f90}
\begin{itemize}
