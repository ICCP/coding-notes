...
\author{Connor Glosser, Jos Seldenthuis, Chris Verzijl, \\ 
  ...
  \titlepic{
    \begin{figure}[b]
      \centering
      \includegraphics[width=0.45\textwidth]{MSU_logo.pdf}\\\vspace{2cm}
      \includegraphics[width=0.45\textwidth]{TU_logo.pdf}
    \end{figure}
  }
\maketitle
\tableofcontents
...
\lstinputlisting{examples/myProg.f90}
...
\lstinputlisting{examples/snppic.f}
...
\lstinputlisting{examples/SumsBeforeGauss.f90}
...
\lstinputlisting{examples/SumsBeforeGauss_subroutine.f90}
...
\lstinputlisting{examples/SumsBeforeGauss_function.f90}
...
\lstinputlisting{examples/smart_sum_function.f90}
...
\lstinputlisting{examples/gauss.f90}
...
\lstinputlisting{examples/Sums.f90}
...
\lstinputlisting[language=,showtabs=true]{examples/example_makefile}
...
\chapter[Debugging]{Debugging; or, where you'll be spending 90\% of your time}
...
\end{lstlisting}
    or by simply writing \texttt{c = a + b}.
...
%Alternatively, the velocity Verlet algorithm has a global error of $\mathcal{O}\left(h^2\right)$, but it is symplectic, \emph{i.e.}, it is time-reversible and conserves (a discrete version of the) energy.
%This makes it ideal for, \emph{e.g.}, molecular dynamics simulations.
...
You can always write your \href{http://xkcd.com/221/}{own}, but Fortran has a few reasonably good subroutines to do it for you.
The default \texttt{gfortran} installation provides access to a low-level \texttt{\keyword{rand}()} function, but you'll almost certainly want to use the more sophisticated \keyword{random\_number} subroutine instead---it generates much higher quality random numbers and automatically threads over arrays.
...
\lstinputlisting{examples/init_random_seed.f90}
...
\lstinputlisting{examples/box_muller.f90}
...
\lstinputlisting{examples/invertMatrix.f90}

\lstinputlisting{examples/calculateEigensystem.f90}
...
\lstinputlisting{examples/PLplotDemo.f90}
\begin{itemize}
...
\lstinputlisting[firstline=13, firstnumber=13, caption=, nolol]{examples/PLplot3D.f90}
...
\begin{itemize}
  ...
  \item \texttt{git commit -m ``Initial commit''} takes everything in the staging area and adds it to the history with the description ``Initial commit''.
\end{itemize}
